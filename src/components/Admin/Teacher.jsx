...
export default function Teacher() {
    ...
    const hdlViewSchedule = async (userId) => {
        navigate(`/${role}/teachers/schedule/${userId}`)
    }
    ...
    return (
        <div className='max-w-[80rem] mx-auto mt-3 select-none'>
                    ...
                    <table className='table text-black'>
                        ...
                        <tbody className='text-center'>
                            {users.filter(user => user.user_role === "TEACHER").map((user, number) => (
                                <tr className='hover:bg-[#FF90BC] hover:rounded-2xl hover:text-white hover:cursor-pointer' onClick={ () => hdlViewSchedule(user?.user_id)} key={user.user_id}>
            ...
            <div className="bg-white py-1 px-2 rounded-2xl max-w-[53rem] mt-5 mx-auto block md:hidden">
                ...
                {users.filter(user => user.user_role === "TEACHER").map((user, number) => (
                    <div className={`animate-slidein opacity-0 [--slidein-delay:200ms] my-2 flex justify-between font-bold text-white px-2 py-2 rounded-lg shadow-md ${user.user_nameprefix === "นางสาว" || user.user_nameprefix === "นาง" ? "bg-[#FF90BC]" : "bg-[#6096B4]"} hover:cursor-pointer`} onClick={ () => hdlViewSchedule(user?.user_id)}  key={number}>
